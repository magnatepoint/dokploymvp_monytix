...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'http://34.14.136.76:8001'
...
// Fetch accounts from backend (derived from transaction data)
export async function fetchAccounts(session: Session): Promise<Account[]> {
  const response = await fetchWithAuth<{ accounts: Account[] }>(session, '/v1/spendsense/accounts')
  const list = response?.accounts ?? []
  return list.map((a: Account & { last_txn_date?: string }) => ({
    id: a.id,
    ...
    last_updated: a.last_txn_date ? new Date(a.last_txn_date).toISOString() : null,
  }))
}

/** @deprecated Use fetchAccounts instead */
