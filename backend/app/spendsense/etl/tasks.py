    ...
    try:
        ...
        await conn.execute(
            ...
            SELECT DISTINCT ON (
                ...
                TRIM(REGEXP_REPLACE(COALESCE(m.normalized_name, ''), '^\\d+\\s+', ''))
            )
                m.user_id,
                m.upload_id,
            ...
            WHERE NOT EXISTS (
                ...
                WHERE tf.user_id = m.user_id
                    ...
                    AND TRIM(REGEXP_REPLACE(COALESCE(tf.merchant_name_norm, ''), '^\\d+\\s+', ''))
                        = TRIM(REGEXP_REPLACE(COALESCE(m.normalized_name, ''), '^\\d+\\s+', ''))
            )
            ORDER BY m.user_id, m.txn_date, m.amount, m.direction, COALESCE(m.raw_txn_id, ''),
                TRIM(REGEXP_REPLACE(COALESCE(m.normalized_name, ''), '^\\d+\\s+', '')),
                LENGTH(COALESCE(m.normalized_name, '')), m.normalized_name
            """,
            batch_id,
        )
