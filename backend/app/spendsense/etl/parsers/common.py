...
def _parse_amount_value(value: Any) -> float | None:
    """Parse a cell value as numeric amount. Returns None if blank or invalid."""
...
def _collapse_pdf_continuation_rows(df: pd.DataFrame) -> pd.DataFrame:
    ...
    When a row has blank date but a distinct amount (from merged date cells),
    treat it as a new transaction (reuse previous date) instead of merging.
    This prevents multiple same-day transactions from collapsing into one.
    """
    if df.empty:
        return df
    ...
    for column in df.columns:
        ...
        if canonical in ("withdrawal_amt", "deposit_amt"):
    ...
    def _has_distinct_amount(row: dict[str, Any]) -> bool:
        """True if row has a non-blank, non-zero amount (withdrawal or deposit)."""
        for col in amount_columns:
            amt = _parse_amount_value(row.get(col))
    ...
    for _, row in df.iterrows():
        ...
        # Plan fix: if continuation row has distinct amount, treat as new transaction
        # (reuse previous date) instead of merging - avoids merged date cell under-count
        if _has_distinct_amount(dict(row)):
            if current_row is not None:
                merged_rows.append(current_row)
            # Start new row: reuse previous date, use this row's values
            prev_date = current_row.get(date_column)
            new_row = {col: row[col] for col in df.columns}
            new_row[date_column] = prev_date
            current_row = new_row
            continue
...
def structure_dataframe(df_raw: pd.DataFrame, *, is_pdf: bool) -> pd.DataFrame:
    ...
    if found_header_row is not None:
        sample_cols = df_raw.iloc[found_header_row].astype(str).tolist()
        logger.info(
            "structure_dataframe: header at row %d, columns: %s",
            found_header_row,
            sample_cols[:10] if len(sample_cols) > 10 else sample_cols,
        )
        df = df_raw.copy()
        df.columns = df.iloc[found_header_row].astype(str)
        df = df.iloc[found_header_row + 1 :].reset_index(drop=True)
        logger.debug("structure_dataframe: data rows after header: %d", len(df))
        if is_pdf:
            df = _expand_multiline_rows(df)
            logger.debug("structure_dataframe: after _expand_multiline_rows: %d rows", len(df))
            df = _collapse_pdf_continuation_rows(df)
            logger.debug("structure_dataframe: after _collapse_pdf_continuation_rows: %d rows", len(df))
        else:
...
def dataframe_to_records(df: pd.DataFrame, *, bank_code: str | None = None) -> list[dict[str, Any]]:
    logger.debug("dataframe_to_records: input rows: %d", len(df))
    df = _normalize_columns(df)

    dates = pd.to_datetime(df["txn_date"], errors="coerce", dayfirst=True)
    invalid_dates = dates.isna()
    dropped_dates = invalid_dates.sum()
    if invalid_dates.any():
        df = df[~invalid_dates].copy()
        dates = dates[~invalid_dates]
        logger.info("dataframe_to_records: dropped %d rows with invalid date", dropped_dates)
        if len(df) == 0:
    ...
    if "amount" not in df.columns:
        ...
        if withdrawal_col is not None or deposit_col is not None:
            ...
            dropped_zero = mask_summary.sum()
            if dropped_zero > 0:
                logger.info("dataframe_to_records: dropped %d rows with both withdrawal and deposit 0", dropped_zero)
    ...
    dropped_amounts = invalid_amounts.sum()
    if invalid_amounts.any():
        df = df[~invalid_amounts].copy()
        logger.info("dataframe_to_records: dropped %d rows with invalid amount", dropped_amounts)
        if len(df) == 0:
    ...
    logger.debug("dataframe_to_records: output records: %d", len(records))
    return records
