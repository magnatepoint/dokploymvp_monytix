...
@router.get("/accounts", response_model=AccountsListResponse, summary="List accounts from transaction data")
async def get_accounts(
    user: AuthenticatedUser = Depends(get_current_user),
    service: SpendSenseService = Depends(get_service),
) -> AccountsListResponse:
    """Return accounts derived from transaction data, grouped by bank_code and account_ref."""
    accounts = await service.list_accounts(user.user_id)
